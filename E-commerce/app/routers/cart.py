...
async def update_cart_item(item_id: int, quantity: int, db: db_dependency = None, current_user: models.User = Depends(get_current_user)):
	user_cart = get_or_create_cart(db, current_user.id)
	item = db.query(models.CartItem).filter(models.CartItem.id == item_id, models.CartItem.cart_id == user_cart.id).first()
	if not item:
