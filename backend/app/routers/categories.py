...
async def create_category(category: schemas.CategoryCreate, db: db_dependency):
...
async def update_category(category_id: int, category: schemas.CategoryUpdate, db: db_dependency):
	c = db.query(models.Category).filter(models.Category.id == category_id).first()
	if not c:
		raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found")
	if category.name:
